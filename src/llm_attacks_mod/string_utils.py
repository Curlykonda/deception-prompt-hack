...
class SuffixManager_split:
    ...
    def get_prompt(self, adv_string=None):
        ...
        if self.conv_template.name == "llama-2":
            assert False, "Not LLama2 conv_template implemented yet."
            ...
            # self.conv_template.update_last_message(f"{self.instruction_1}")
            # toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            # self._goal_slice_1 = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks)))

            # self.conv_template.update_last_message(f"{self.instruction_1} {self.adv_string}")
            # toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            # self._control_slice = slice(self._goal_slice_1.stop, len(toks))

            # self.conv_template.update_last_message(f"{self.instruction_1} {self.adv_string} {self.instruction_2}")
            # toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            # self._goal_slice_2 = slice(self._control_slice.stop, len(toks))
            ...
            if python_tokenizer:
                ...
                assert False, "Have to use PYthon Tokenizer."
                ...
                # self._goal_slice_1 = slice(
                #     encoding.char_to_token(prompt.find(self.instruction_1)),
                #     encoding.char_to_token(prompt.find(self.instruction_1) + len(self.instruction_1))
                # )
                # self._control_slice = slice(
                #     encoding.char_to_token(prompt.find(self.adv_string)),
                ...
                # self._goal_slice_2 = slice(
                #     encoding.char_to_token(prompt.find(self.instruction_2)),
                #     encoding.char_to_token(prompt.find(self.instruction_2) + len(self.instruction_2))
                # )
                # self._assistant_role_slice = slice(
                #     encoding.char_to_token(prompt.find(self.conv_template.roles[1])),
